...
class DQNAgent(BaseAgent):
 ...
 epsilon-greedy：这是一种常见的探索 - 利用策略。对于每个智能体，以 epsilon 的概率随机选择一个动作进行探索，以 1 - epsilon 的概率选择 Q 值最大的动作进行利用。
categorical：使用 numba_categorical_sample 函数根据 Q 值进行分类采样，具体实现取决于 numba_categorical_sample 函数的定义。
epsilon-categorical：使用 numba_epsilon_categorical_sample 函数根据 Q 值和 epsilon 进行带探索的分类采样，同样具体实现取决于该函数的定义。
greedy：对于每个智能体，直接选择 Q 值最大的动作，不进行探索。
        """
        action = []
