...
from modules.qmix import QMIXAgentNetwork, QMIXMixingNetwork
...
class QMIXAgent(BaseAgent):
    """
    QMIX智能体：实现集中式训练+分布式执行(CTDE)架构
    
    CTDE架构说明：
    =============
    
    1. 集中式训练（Centralized Training）：
       - 训练时使用全局状态(state)和混合网络
       - 混合网络将各个智能体的Q值混合为全局Q值
    ...
    3. 网络结构：
       ...
       - mixing_network: 混合网络，将个体Q值混合为全局Q值
    ...
    def __init__(self, obs_dim, act_dim, n_agents, state_dim):
        ...
        # 创建智能体网络（每个智能体共享参数）
        self.agent_network = QMIXAgentNetwork(obs_dim, self.hidden_dim, act_dim).to(device)
        self.target_agent_network = QMIXAgentNetwork(obs_dim, self.hidden_dim, act_dim).to(device)
        self.target_agent_network.load_state_dict(self.agent_network.state_dict())
        
        # 创建混合网络
        self.mixing_network = QMIXMixingNetwork(n_agents, state_dim, self.hidden_dim).to(device)
        self.target_mixing_network = QMIXMixingNetwork(n_agents, state_dim, self.hidden_dim).to(device)
    ...
    def cal_q_loss(self, sample, losses, log_vars=None, global_steps=None):
        """
        QMIX集中式训练：使用全局状态和混合网络计算损失
        
        集中式训练的特点：
        1. 使用全局状态(state)作为混合网络的输入
        2. 混合网络将各个智能体的Q值混合为全局Q值
        3. 训练目标是使全局Q值接近全局TD目标
        """
        ...
        next_state = sample.get('next_state', None)  # [batch_size, state_dim] - 下一全局状态
        ...
        # 集中式训练：使用混合网络将个体Q值混合为全局Q值
        # 混合网络以全局状态为条件，确保满足单调性约束
        total_q = self.mixing_network(chosen_action_qvals, state)  # [batch_size, 1]
        total_target_q = self.target_mixing_network(targets, next_state)  # [batch_size, 1]
    ...
    def update_target(self):
        ...
        if hparams['soft_update_target_network']:
            ...
            soft_update(self.mixing_network, self.target_mixing_network, hparams['tau'])
        else:
            hard_update(self.agent_network, self.target_agent_network)
            hard_update(self.mixing_network, self.target_mixing_network)