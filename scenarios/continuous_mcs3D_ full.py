...
class ContinuousMCS3D:
    def __init__(self, n_agent=N_AGENT, world_scale_2D=(NUM_GRIDS, NUM_GRIDS),
        ...
        poi_info_dim = self.num_poi * 5  # 每个POI: 2绝对位置 + 3相对信息 + 2覆盖状态
        self_state_dim = 3 + 3 + 1 + 1 + 1 + 1  # 位置3 + 速度3 + 能源1 + 覆盖半径1 + 覆盖数量1 + 动作空间1
        other_uav_dim = (self.n_agent - 1) * 6  # 每个其他无人机: 3相对位置 + 1覆盖数量 + 1覆盖半径 + 1能量
        global_info_dim = 5  # 全局覆盖信息
        adj_info_dim = self.n_agent  # 邻接矩阵行
        
        self.obs_dim = poi_info_dim + self_state_dim + other_uav_dim + global_info_dim + adj_info_dim
    ...
    def get_obs(self):
        ...
        for i, (x, y, z) in enumerate(self.pos_agent):
            obs_i = []

            # 1. 所有POI的覆盖相关信息
            all_poi_info = []
            for j, (poi_x, poi_y) in enumerate(self.poi_pos):
                
                # POI相对于当前无人机的位置和距离
                rel_x = poi_x - x
                rel_y = poi_y - y
                distance = math.sqrt(rel_x ** 2 + rel_y ** 2)
                all_poi_info.extend([rel_x, rel_y, distance])
                
                # POI的覆盖状态（0:未覆盖, 2:被当前无人机覆盖）
                coverage_status = 2 if self.delta_list[i, j] == 2 else 0
                all_poi_info.append(coverage_status)
                
                # POI是否被任何无人机覆盖
                is_covered_by_any = 1 if self.poi_is_covered[j] > 0 else 0
                all_poi_info.append(is_covered_by_any)
            
            obs_i.append(all_poi_info)
            ...
            # 覆盖能力
            obs_i.append([self.rcov[i]])  # 覆盖半径
            obs_i.append([self.agent_cover[i]])    # 覆盖POI数量
            
            # 当前动作能力
            obs_i.append([self.agents.n_action])   # 动作空间大小
            ...
            for j in range(self.n_agent):
                if i != j:  # 排除自身
                    # 相对位置
                    rel_x = self.agents.x[j] - x
                    rel_y = self.agents.y[j] - y
                    rel_z = self.agents.z[j] - z
                    other_uav_info.extend([rel_x, rel_y, rel_z])
                    
                    # 覆盖状态
                    other_uav_info.append(self.agent_cover[j])    # 覆盖POI数量
                    other_uav_info.append(self.rcov[j])           # 覆盖半径
                    
                    # 能量状态
                    guiyi_other_energy = (self.agents.energy_consumption[j]-self.agents.hover_energy_cost)/(1-self.agents.hover_energy_cost)
                    other_uav_info.append(guiyi_other_energy)
            
            obs_i.append(other_uav_info)

            # 4. 全局覆盖信息
            global_info = [
                np.sum(self.poi_is_covered) / self.num_poi,  # 总覆盖率
                np.mean(self.agent_cover),                   # 平均覆盖POI数
                np.mean(self.rcov),                          # 平均覆盖半径
                self.timeslot / hparams['episode_length'],   # 时间进度
                np.sum(self.poi_is_covered),                 # 总覆盖POI数
            ]
            obs_i.append(global_info)

            # 5. 邻接矩阵信息（当前无人机所在行）
            adj_row = self.get_adj()[i].tolist()
            obs_i.append(adj_row)

            obs_i = np.concatenate(obs_i).astype(np.float32)
            obs.append(obs_i)
